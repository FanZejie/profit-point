export function calculateTrading(currentPrice: number, stopLossPrice: number, leverage: number, investment: number): { [key: string]: number } {
...
const calculatePoints = (currentPrice: number, stopLossPrice: number, isUpward: boolean): { [key: string]: number } => {
    const lossRange: number = Math.abs(currentPrice - stopLossPrice); // 获取波动范围
    const directionMultiplier: number = isUpward ? 1 : -1; // 根据方向选择加减

    // 计算各止盈点
    const winRange1: number = parseFloat((currentPrice + directionMultiplier * lossRange).toFixed(8));
    const winRange2: number = parseFloat((currentPrice + directionMultiplier * lossRange * 2).toFixed(8));
    const winRange3: number = parseFloat((currentPrice + directionMultiplier * lossRange * 3).toFixed(8));
...
// 计算亏损金额
const calculateLoss = (currentPrice: number, stopLossPrice: number, leverage: number, investment: number): number => {
    const lossRange: number = Math.abs(currentPrice - stopLossPrice); // 获取波动范围
    const ethAmount: number = investment / currentPrice; // 计算买入的ETH数量
    const lossAmount: number = lossRange * ethAmount * leverage; // 计算亏损金额
    return parseFloat(lossAmount.toFixed(8)); // 返回亏损金额，保留两位小数
};
...
export function calculateExitPrice(
    ...
    return parseFloat(res.toFixed(8)); // 返回保留两位小数的价格
  }